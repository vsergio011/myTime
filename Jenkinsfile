...
"""
  ...
  stages {
    stage('Build and push image with Container Builder') {
      steps {
        container('gcloud') {
          sh "PYTHONUNBUFFERED=1 gcloud builds submit -t gcr.io/${PROJECT}/apitasks/dockerfile ."
        }
      }
    }
    stage('Deploy Canary') {
      ...
      steps {
        container('kubectl') {
          ...
          sh("sed -i.bak 's#gcr.io/cloud-solutions-images/gceme:1.0.0#${IMAGE_TAG}#' ./apitasks/k8s/canary/*.yaml")
          ...
          step([$class: 'KubernetesEngineBuilder', namespace:'production', projectId: env.PROJECT, clusterName: env.CLUSTER, zone: env.CLUSTER_ZONE, manifestPattern: 'apitasks/k8s/canary', credentialsId: env.JENKINS_CRED, verifyDeployments: true])
